...
위의 그림에서 p는 a라는 변수를 가리키는 포인터 변수이다. 그림에 해당되는 C언어
문장은 다음과 같다.
...
p가 a를 가리키게 하려면 a의 주소를 p에 대입한다. 변수의 주소는 & 연산자를 변수에
적용시켜서 추출할 수 있다.
...
즉 *p와 변수 a는 전적으로 동일하다. 즉 이들의 값만 같은 것이 아니고 동일한 객체를 가리키기 때문에 *p의 값을 변경하게 되면 변수 a의 값도 바뀌게 된다.

###포인터와 관련된 연산자

---

 포인터에서 가장 중요한 연산자는 &와 *이다. 그림과 같이 & 연산자는 변수로부터
변수의 주소를 추출해내는 연산자이다. 어떤 변수에도 적용할 수 있다.
...
{% endhighlight %}
 ...
 void *p는 아무것도 가리키지 않는 포인터를 의미한다. void 포인터는 필요할 때마다 다른 포인터로 바꾸어서 사용하는 것이 가능하다.
...
### 포인터의 포인터
포인터도 하나의 변수이므로 포인터의 포인터도 선언이 가능하다. 실제로 앞으로 다룰 몇 개의 자료 구조에서 함수에 포인터의 포인터를 전달할 것이다. 다음의 문장은 문자 포인터에 대한 포인터를 선언하고 연결한다.
...
###포인터에 대한 연산
 포인터에 대한 연산은 보통의 연산과 다른 의미를 지닌다. 정수의 포인터에 값을 더하거나 빼보자. 다음과 같이 정수의 포인터 pi를 생성하고 정수 변수를 가리키도록 하자.

pi+1, pi-1의 값은 어떻게 될까?  주소 값이 하나 감소되고 증가하는 것이 아니라 pi-1은 pi가 가리키는 객체 하나 뒤에 있는 객체를 가리키고 pi+1은 pi가 가리키는 객체 하나 앞에있는 객체를 가리킨다. 즉 앞의 코드에서는 pi+1은 A[4]를 pi-1은 A[2]를 가리킨다.
